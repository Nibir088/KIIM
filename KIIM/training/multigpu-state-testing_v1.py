...
@hydra.main(config_path="/project/biocomplexity/wyr6fx(Nibir)/IJCAI-25_Irrigation_Mapping/Pytorch-Lightening/KIIM/config", config_name="multigpu-state-testing-baseline_v1", version_base="1.2")
def train(cfg: DictConfig) -> None:
    ...
    ##### read best_params ###########
    best_params = OmegaConf.load(Path('hparam_tuning/results/optimization_results.yaml')).best_params
    ...
    if not cfg.model.loss_config.use_ce:
        cfg.model.loss_config = {
            "ce_weight": 0.0,
             ...
             "stream_weight": 1- best_params['dice_weight']-best_params['focal_weight']-best_params['kg_weight']
        }
...
#     # Initialize loggers
    ...
    try:
        ...
        if cfg.train.save_model:
            trainer.save_checkpoint(save_dir / "final_model_2.ckpt")
