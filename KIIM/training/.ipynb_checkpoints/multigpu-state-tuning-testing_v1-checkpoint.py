...
@hydra.main(config_path="/project/biocomplexity/wyr6fx(Nibir)/IJCAI-25_Irrigation_Mapping/Pytorch-Lightening/KIIM/config", config_name="multigpu-state-testing-KIIM-with-state-tuning_v1", version_base="1.2")
def train(cfg: DictConfig) -> None:
    ...
    model = KIIM.load_from_checkpoint(cfg.test.ckpt, **cfg.model)
...
#     # Initialize loggers
    ...
    filename="{epoch}-{val_iou_macro:.3f}" if has_validation else "{epoch}-{train_loss:.3f}",
    monitor="val_iou_macro_irr" if has_validation else "train_loss",
    mode="max" if has_validation else "min",
    save_top_k=5,  # Ensures top 5 best models are saved
    save_last=True,  # Always keeps the last checkpoint
    )
    callbacks.append(checkpoint_callback)
    
    # Early stopping callback
    if has_validation and cfg.train.early_stopping:
    ...
    trainer = pl.Trainer(
        ...
        check_val_every_n_epoch=cfg.train.check_val_every_n_epoch if has_validation else None,
    ...
    try:
        ...
        if best_model_path:
            ...
            trainer.test(ckpt_path=save_dir / "final_model.ckpt", datamodule=data_module)
